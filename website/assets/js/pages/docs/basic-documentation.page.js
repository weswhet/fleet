parasails.registerPage('basic-documentation', {
  ...
  mounted: async function() {
    ...
    // Setting a flag to determine if the user's browser navigator.clipboard.writeText() method.
    let isCopyingLinksSupported = _.contains(_.methods(navigator.clipboard), 'writeText');

    if(isCopyingLinksSupported) {
      ...
      for(let key in Object.values(headingsOnThisPage)){
        ...
        $(heading).click(()=> {
          ...
          if(!$(heading).hasClass('copied') && isCopyingLinksSupported){
